...
from libs.combobox import ComboBox   #######changing search filter here
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        listLayout.addWidget(self.labelcount)
        ...
        self.labelList.itemChanged.connect(self.labelItemChanged) ####using this selected item's bounding box is showing and others are getting removed
        # listLayout.addWidget(self.labelList)   ####to show the annotation label names below bounding box
        listLayout.addWidget(self.labelList)   ####to show the annotation label names below bounding box
...
# 
    ...
    def updateComboBox(self):
        ...
        # Add a null row for showing all the labels   #####showing all levels
    ...
    def comboSelectionChanged(self, index):
        ...
        for i in range(self.labelList.count()):
            if text == "":
                ...
                self.labelList.item(i).setCheckState(0) ######to filter according to search selection in dropdown box
                self.labelList.item(i).setHidden(True)

            else:
                self.labelList.item(i).setCheckState(2) #####to enable more than one time selection change in the search filter dropwdown box
                self.labelList.item(i).setHidden(False)
                count = count + 1
        self.count = count
        if count > 0:
            labelcount.setText('total count = ' + str(count))
        # print(count)
    ...
    def scanAllImages(self, folderPath):
        extensions = ['.%s' % fmt.data().decode("ascii").lower() for fmt in QImageReader.supportedImageFormats()] #may need to change here also the file type
        images = []
    ...
    def openFile(self, _value=False):
        ...
        formats = ['*.%s' % fmt.data().decode("ascii").lower() for fmt in QImageReader.supportedImageFormats()] ############edit format here may be
